...
def _collect_bumeran(
    ...
    def _extract_with(scraper_inst: BumeranScraper, wait: float) -> List[JobRecord]:
        ...
        logger.info("[bumeran] Esperando %.1f s para carga inicial", wait)
        time.sleep(wait)
        return scraper_inst.extraer_todos_los_puestos(timeout=10, page_wait=page_wait)
...
def _collect_computrabajo(
    ...
    def _extract_with(scraper_inst: ComputrabajoScraper, wait: float) -> List[JobRecord]:
        ...
        scraper_inst.buscar_vacante(busqueda)
        logger.info("[computrabajo] Esperando %.1f s para carga inicial", wait)
        time.sleep(wait)
        return scraper_inst.extraer_todos_los_puestos(timeout=12, page_wait=page_wait)
...
def collect_jobs(
    ...
    if executed:
        ...
        for src in executed:
            kept = sum(1 for job in combined if (job.get("fuente") or "").lower() == src)
